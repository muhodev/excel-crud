const KullanilmisTakim = require('../model/KullanilmisTakim')
exports.getBilenmisTakimListesi = async (req, res) => {
    ...
    //     console.log(yachtType,"type backend");
        const result = await KullanilmisTakim.find({});
        res.json({
            success: true,
            result
        })
...
exports.createBilenmisTakim = async (req, res) => {
    try {
        ...
        const takim = new KullanilmisTakim(req.body);
...
exports.deleteBilenmisTakim = async (req, res) => {
    try {
        ...
        const result = await KullanilmisTakim.findByIdAndRemove(id)
        res.json({
            success: true,
            result
        })
...
exports.updateBilenmisTakim = async (req, res) => {
    try {
        ...
        const result = await KullanilmisTakim.findByIdAndUpdate(req.params.id, updateData);
        res.json({
            result
        })
