const Page3BilenmisTakim = require('../model/Page3BilenmisTakim')
exports.getBilenmisTakimListesi = async (req, res) => {
    ...
    //     console.log(yachtType,"type backend");
        const result = await Page3BilenmisTakim.find({});
        res.json({
            success: true,
            result
        })
...
exports.createBilenmisTakim = async (req, res) => {
    try {
        ...
        const takim = new Page3BilenmisTakim(req.body);
...
exports.getBilenmisTakim = async (req, res) => {
    try {
        ...
        const result = await Page3BilenmisTakim.findById(id)
        res.json({
            success: true,
            result
        })
...
exports.deleteBilenmisTakim = async (req, res) => {
    try {
        ...
        const result = await Page3BilenmisTakim.findByIdAndRemove(id)
        res.json({
            success: true,
            result
        })
...
exports.updateBilenmisTakim = async (req, res) => {
    try {
        ...
        const result = await Page3BilenmisTakim.findByIdAndUpdate(req.params.id, updateData);
        res.json({
            result
        })
